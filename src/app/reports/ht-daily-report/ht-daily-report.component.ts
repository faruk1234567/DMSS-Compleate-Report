...
@Component({
  selector: 'app-ht-daily-report',
  templateUrl: './ht-daily-report.component.html',
  styleUrls: ['./ht-daily-report.component.scss']
})
export class HtDailyReportComponent implements OnInit, OnDestroy {
  ...
  onSearchHoldingNoChange(): void {
    const searchTerm = this.formData.searchHoldingNo.toLowerCase().trim();

    if (searchTerm) {
      this.reportData = this.originalReportData.filter(item =>
        item.holdingNo && String(item.holdingNo).toLowerCase().includes(searchTerm)
      );
    } else {
  ...
  getReport(): void {
    ...
    const url = `http://localhost:8080/api/daily_Recovery_report/holding-tax?municipalityId=${municipalityId}&startDate=${adjustedStartDate}&endDate=${adjustedEndDate}`;
